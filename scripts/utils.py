...
class BeautyThemeGenerator:
    ...
    def _initialize_base_themes(self) -> list:
        """基本的な美容業界テーマ（ビジネス・マーケティング・生成AI特化）"""
        return [
            # AI・テクノロジー活用系
            "美容師のためのChatGPT活用完全ガイド",
            "AIでサロンマーケティングを自動化する方法",
            "生成AIでSNS投稿を量産するテクニック",
            "Claudeで顧客メールを個別最適化する術",
            "Midjourneyでサロンイメージを作成する方法",
            "AIツールで予約管理を効率化",
            "プロンプトエンジニアリング入門",
            "AI音声アシスタントのサロン活用術",
            
            # Instagram集客特化
            "Instagramリールで1万再生を獲得する法則",
            "フォロワー1000人突破の具体的戦略",
            "Instagramストーリーズで予約を取る方法",
            "ハッシュタグ最適化でリーチを2倍にする",
            "DMマーケティングでリピート率向上",
            "インスタライブでファンを作る方法",
            "UGCを活用した口コミマーケティング",
            ...
            # ローカルビジネス経営戦略
            "Googleビジネスプロフィールで集客する秘訣",
            "地域密着型サロンのMEO対策完全版",
            "LINE公式アカウントでリピート率5倍に",
            "顧客データ分析で売上30%アップの手法",
            "サロン経営の数字管理で利益率2倍に",
            "キャッシュレス決済導入のメリット",
            "サブスクモデルで安定収益を作る",
            ...
            # TikTok・YouTube活用
            "TikTokでバズる美容系ショート動画の作り方",
            "YouTubeショートで新規顧客を獲得する方法",
            "動画マーケティングで40代女性を掃きつける",
            "バイラル動画の法則性と再現性",
